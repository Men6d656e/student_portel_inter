...
export default function UploadResultPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!formData.subject || !formData.resultType || !formData.totalMarks || !formData.degree || !formData.class || !formData.startYear || !formData.endYear) {
        ...
        try {
            ...
            const extractedData = await processResultFile(selectedFile, (progress) => {
            ...
            const invalidMarks = extractedData.filter((item) => item.marks > totalMarks)
            if (invalidMarks.length > 0) {
                toast.error(`Some students have marks exceeding total marks (${totalMarks})`)
                setIsProcessing(false)
                return
            }
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Select onValueChange={(val) => setFormData({ ...formData, resultType: val })} value={formData.resultType}>
                                    <SelectTrigger>
                        ...
                        {isProcessing && (
                            <div className="space-y-2">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm">Processing file... {processingProgress}%</span>
                                </div>
                            </div>
                        )}
