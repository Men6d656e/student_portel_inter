...
export const teacherRouter = router({
  getStats: adminProcedure.query(async ({ ctx }) => {
    const totalTeachers = await ctx.db.user.count({
      where: { role: "TEACHER" },
    });
    const verifiedTeachers = await ctx.db.user.count({
      where: { role: "TEACHER", emailVerified: true },
    });
    const unverifiedTeachers = await ctx.db.user.count({
      where: { role: "TEACHER", emailVerified: false },
    });
  ...
  getTeachersNeedAprovel: adminProcedure.query(async ({ ctx }) => {
  ...
  delete: adminProcedure
    .input(z.object({ id: z.string() }))
    .mutation(async ({ ctx, input }) => {
      await ctx.db.result.deleteMany({ where: { uploadedById: input.id } });
      return ctx.db.user.delete({ where: { id: input.id } });
    }),
  approvedTeacherIdentity: adminProcedure
    .input(z.object({ id: z.string() }))
    .mutation(async ({ ctx, input }) => {
      return ctx.db.user.update({
        where: { id: input.id },
